...
class BayesianEstimation():
    ...
    def random_intercept_param(self, group_variable, group_count):
        ...
        return [
               ...
               "lambda {0}, {1}: tfd.Independent(tfd.Normal(loc=affine(tf.ones([{2}]), {3}[..., tf.newaxis]),scale_identity_multiplier={4}),reinterpreted_batch_ndims=1)".format(
                       "sigma_intercept_"+group_variable,
                       "mu_intercept_"+group_variable,
                       group_count,
                       "mu_intercept_"+group_variable,
    ...
    def target(self, args, loc, scale):
        return [
            "lambda {}:  tfd.Independent(tfd.Normal(loc=affine(tf.ones([{2}]), {3}[..., tf.newaxis]),scale_identity_multiplier={4}),reinterpreted_batch_ndims=1)".format(args,loc,scale)
        ]
